...
#### Data from `r cycle_config$cycle_label` {#subtitle}
...
college_q2 <- question_list$Q2 |>
  dplyr::filter(`Institution Name` == params$college) |>
  tidyr::pivot_longer(
...
plot_reporting <- reporting_data |>
  dplyr::left_join(college_q2 |> dplyr::mutate(your_school = TRUE), by = "Question") |>
  dplyr::mutate(
    your_school = tidyr::replace_na(your_school, FALSE),
    fill_color = dplyr::if_else(your_school, "#217DBB", "#7CBCE8")
  )

plot_reporting |>
  ggplot(aes(reorder(reporting_option, freq), freq, fill = fill_color)) +
  ...
  geom_point(
    data = dplyr::filter(plot_reporting, your_school),
    aes(y = freq),
    shape = 21,
    fill = "black",
    colour = "white",
...
matrixPlot( breaks = c(0,5,10,15), 
           font = params$plotfont,
           size = 16, college = params$college)
```
...
q17_custom <- question_list[[alumni_services_scope]] |>
  filter(`Institution Name` == params$college) |>
  select(all_of(alumni_services_scope)) |>
  rename(value = all_of(alumni_services_scope)) |>
...
**Published `r format(Sys.Date(), "%B %Y")`, Data Analysis completed by St Olaf College**
