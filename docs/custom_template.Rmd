...
# load fonts (wrapped in tryCatch for robustness)
tryCatch({
  font_add_google(params$plotfont)
}, error = function(e) {
  message("Note: Could not download font. Using defaults.")
})
...
load("../lacn.RData", envir = .GlobalEnv)
```

#### Data from 2025-2026 {#subtitle}
...
enroll_data |>
  ...
  ggrepel::geom_text_repel(
    ...
    theme(
      ...
      text = element_text(size = 25, family = params$plotfont)
    )
```
...
# Q2 is now multi-select, use value column for bar chart
college_q2 <- question_list$Q2 |>
  dplyr::filter(`Institution Name` == params$college) |>
  tidyr::pivot_longer(cols = starts_with("Q2_"), names_to = "Question", values_to = "response") |>
  dplyr::filter(!is.na(response) & response != "") |>
  dplyr::left_join(dplyr::distinct(response_key[response_key$main=="Q2", c("Question","dim2")]), by = "Question") |>
  dplyr::select(dim2) |>
  dplyr::rename(value = dim2)

ggplot2::ggplot(data = reporting_data |> dplyr::filter(n > 1),
                mapping = ggplot2::aes(reorder(value, freq), freq)) +
  ggplot2::geom_hline(yintercept = seq(0,1,by=0.2), colour = "grey") +
  ggplot2::geom_bar(width = 0.6, stat = 'identity', fill = "#7CBCE8") +
  {
    if(nrow(college_q2) > 0) {
      ggplot2::geom_bar(data = dplyr::filter(reporting_data, value %in% college_q2$value & n > 1),
                        width = 0.6, stat = 'identity', fill = "#217DBB")
    }
  } +
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(title = NULL, x = NULL, y = "\nFrequency") +
  ggplot2::theme(
...
<!--   theme( -->
<!--     panel.background = element_blank(), -->
<!--     text = element_text(family = params$plotfont, size = 22), -->
...
matrixPlot( breaks = c(0,5,10,15), 
           font = params$plotfont,
           size = 25, college = params$college)
```
...
all_list$single$Q14 |>
  ...
  theme(
    text = element_text(family = params$plotfont, size = 25),
    panel.background = element_blank(),
    axis.ticks = element_blank(),
...
appt_alum_data |>
  ...
  ggplot2::theme(
    text = ggplot2::element_text(size = 22, family = params$plotfont),
    axis.text.x = ggplot2::element_blank(),
    axis.ticks = ggplot2::element_blank(),
    panel.background = ggplot2::element_blank(),
    legend.title = ggplot2::element_blank(),
...
sengage_year_data |>
  ...
  ggplot2::theme(
    text = ggplot2::element_text(size = 25, family = params$plotfont),
    axis.ticks = ggplot2::element_blank(),
    axis.text.x = ggplot2::element_text(size = 16, lineheight = 0.5, colour = "black"),
    panel.background = ggplot2::element_blank(),
    legend.title = ggplot2::element_blank(),
...
sengage_type_data |>
  ...
  ggplot2::theme(
    text = ggplot2::element_text(size = 22, family = params$plotfont),
    axis.ticks = ggplot2::element_blank(),
    axis.text.x = ggplot2::element_text(size = 12, colour = "black", angle = 25, hjust = 1),
    panel.background = ggplot2::element_blank(),
    legend.title = ggplot2::element_blank(),
    legend.text = element_text(size = 16),
    legend.position = "top",
...
endow_exp_data |>
 ...
 # ggplot2::scale_y_continuous(labels = #c("0","0.5","1","1.5","$2"))+
  ...
  ggplot2::theme(
    text = ggplot2::element_text(family = params$plotfont, size = 22),
    panel.background = ggplot2::element_blank(),
    axis.text.x = ggplot2::element_blank(),
    axis.ticks = ggplot2::element_blank(),
    legend.title = ggplot2::element_blank(),
...
**Published Feb 2026, Data Analysis completed by St Olaf College**
